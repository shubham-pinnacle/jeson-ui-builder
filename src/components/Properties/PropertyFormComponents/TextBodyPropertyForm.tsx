...
export default function TextBodyPropertyForm({
  ...
  const handleTextItemChange = (index: number, value: string) => {
    ...
    // In markdown mode, we always store as array
    h("text", newTextItems);
  };
  ...
  const maxChars = 4096; // Based on the documentation for TextBody
  const isOverLimit = typeof textContent === "string" ? 
    textContent.length > maxChars : 
    textContent.join("").length > maxChars;
    ...
    <Stack spacing={2}>
      {!isMarkdown ? (
        <TextField
          ...
          size="small"
          error={isOverLimit}
          helperText={`${typeof textContent === "string" ? textContent.length : textContent.join("").length}/${maxChars} characters`}
          FormHelperTextProps={{
            sx: {
